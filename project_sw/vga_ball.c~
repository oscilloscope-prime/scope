...
struct vga_ball_dev {
	...
	void __iomem *virtbase; /* Where registers can be accessed in memory */
	vga_ball_arg_t position; /* Coordinates */
	//vga_ball_color_t background;
} dev;
...
static void write_coordinate(vga_ball_arg_t* position)
{
	iowrite16(position->x, posX(dev.virtbase));
	iowrite16(position->y, posY(dev.virtbase));
	iowrite16(position->r, ri(dev.virtbase));
	iowrite16(position->t, tr(dev.virtbase));
...
static long vga_ball_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
	...
	case VGA_BALL_WRITE_COORD:
		if (copy_from_user(&vla, (vga_ball_arg_t *) arg,
				   sizeof(vga_ball_arg_t)))
			return -EACCES;
		//if (vla.y > 465 || vla.x > 1250 || vla.x < 0 || vla.y < 0)
			//return -EINVAL;
